...
@implementation QWGameDetailLogic

- (void)submitDanmu:(NSString *)key content:(NSString *)content completeBlock:(QWCompletionBlock)aBlock
{
    ...
    QWOperationParam *param = [QWInterface getWithUrl:[NSString stringWithFormat:@"%@submitdanmu/", self.bookVO.url] params:params andCompleteBlock:^(id aResponseObject, NSError *anError) {
...
- (void)getDetailWithBookName:(NSString *)bookName andCompleteBlock:(QWCompletionBlock)aBlock
{
    ...
    QWOperationParam *param = [QWInterface getWithDomainUrl:@"book/match/" params:params andCompleteBlock:^(id aResponseObject, NSError *anError) {
        if (aResponseObject && !anError) {
            BookVO *vo = [BookVO voWithDict:aResponseObject];
            self.bookVO = vo;
            if (aBlock) {
                aBlock(self.bookVO, nil);
            }
        }
        else {
            if (aBlock) {
...
{
    QWOperationParam *param = [QWInterface getWithUrl:[NSString stringWithFormat:@"%@award_rank/", self.bookVO.url] andCompleteBlock:^(id aResponseObject, NSError *anError) {
        if (aResponseObject && !anError) {
...
{
    QWOperationParam *param = [QWInterface getWithUrl:[NSString stringWithFormat:@"%@tip_rank/", self.bookVO.url] andCompleteBlock:^(id aResponseObject, NSError *anError) {
        if (aResponseObject && !anError) {
            UserPageVO *vo = [UserPageVO voWithDict:aResponseObject];
            if (IS_IPHONE_DEVICE && vo.results.count > 3) {
                vo.results = (id)[vo.results subarrayWithRange:NSMakeRange(0, 3)];
...
{
    QWOperationParam *param = [QWInterface getWithUrl:self.bookVO.buy_url andCompleteBlock:^(id aResponseObject, NSError *anError) {
        if (aResponseObject && !anError) {
            if (aBlock) {
                aBlock(aResponseObject, nil);
            }
...
- (void)checkWithCompleteBlock:(QWCompletionBlock)aBlock
{
    ...
    QWOperationParam *param = [QWInterface getWithUrl:self.bookVO.check_url params:params andCompleteBlock:^(id aResponseObject, NSError *anError) {
        if (!anError) {
            self.check = aResponseObject[@"code"];
            if (aBlock) {
                aBlock(aResponseObject, nil);
            }
        }
...
- (void)getFaithPageWithCompleteBlock:(QWCompletionBlock)aBlock {
    ...
    QWOperationParam *param = [QWInterface getWithUrl:[NSString stringWithFormat:@"%@/game/%@/points/",[QWOperationParam currentDomain], self.bookVO.nid] params:params andCompleteBlock:^(id aResponseObject, NSError *anError) {
        if (aResponseObject && !anError) {
...
- (void)getAwardDymicPageWithCompleteBlock:(QWCompletionBlock)aBlock {
    ...
    QWOperationParam *param = [QWInterface getWithUrl:[NSString stringWithFormat:@"%@/game/%@/award_new_feed/",[QWOperationParam currentDomain], self.bookVO.nid] params: params andCompleteBlock:^(id aResponseObject, NSError *anError) {
        if (aResponseObject && !anError) {
            UserPageVO *listVO = [UserPageVO voWithDict: aResponseObject];
