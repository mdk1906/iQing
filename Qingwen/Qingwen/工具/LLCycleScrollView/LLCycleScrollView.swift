...
open class LLCycleScrollView: UIView, UICollectionViewDelegate, UICollectionViewDataSource, UIScrollViewDelegate {
    ...
    //book
    open var bookName: Array<String> = [] {
        didSet {
            if bookName.count > 0 {
                if imagePaths.count == 0 {
                    imagePaths = bookName
    ...
    open class func llCycleScrollViewWithFrame(_ frame: CGRect, imageURLPaths: Array<String>? = [], titles:Array<String>? = [], didSelectItemAtIndex: LLdidSelectItemAtIndexClosure? = nil) -> LLCycleScrollView {
        ...
        llcycleScrollView.userName = []
        llcycleScrollView.content = []
        llcycleScrollView.rank = []
        llcycleScrollView.bookName = []
        llcycleScrollView.stone = []
        llcycleScrollView.medal = []
...
//        if let userNameList = u, titleList.count > 0 {
//            llcycleScrollView.titles = titleList
//        }
    ...
    override open func layoutSubviews() {
        ...
        if customPageControlStyle == .none || customPageControlStyle == .system || customPageControlStyle == .image {
            if pageControlPosition == .center {
                pageControl?.frame = CGRect.init(x: 0, y: 115, width: UIScreen.main.bounds.width, height: 10)
            }else{
                let pointSize = pageControl?.size(forNumberOfPages: self.imagePaths.count)
                if pageControlPosition == .left {
                    pageControl?.frame = CGRect.init(x: (UIScreen.main.bounds.width - (pointSize?.width)! - pageControlLeadingOrTrialingContact) * 0.5-20, y: -17, width: UIScreen.main.bounds.width, height: 10)
                }else{
                    pageControl?.frame = CGRect.init(x: (UIScreen.main.bounds.width - (pointSize?.width)! - pageControlLeadingOrTrialingContact) * 0.5-20, y: -17, width: UIScreen.main.bounds.width, height: 10)
                }
            }
    ...
    open func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if isOnlyTitle && titles.count > 0{
            ...
            if imagePaths.count == 0 {
                ...
                if medal.count != 0{
                   let medalPath = medal[itemIndex]
                    cell.medalImgView.qw_setImageUrlString(QWConvertImageString.convertPicURL(medalPath, imageSizeType: QWImageSizeType.none), placeholder: nil, animation:true)
                }
                else{
                ...
                if imagePath.hasPrefix("http") {
                     cell.imageView.qw_setImageUrlString(QWConvertImageString.convertPicURL(imagePath, imageSizeType: QWImageSizeType.avatar), placeholder: UIImage(named: "mycenter_logo2"), animation:true)
                    
                    let imgBtn = UIButton.init()
                    imgBtn.frame = CGRect(x:10,y:3,width:46,height:46)
                    imgBtn.addTarget(self, action: #selector(imgClick), for: .touchUpInside)
                    cell.contentView.addSubview(imgBtn)
                    
                    let bookBtn = UIButton.init()
                    bookBtn.frame = CGRect(x:10,y:53,width:200,height:17)
                    bookBtn.addTarget(self, action: #selector(bookClick), for: .touchUpInside)
                    cell.contentView.addSubview(bookBtn)
                }else{
                ...
                // 对冲数据判断
                if itemIndex <= userName.count-1 {
                    
                    cell.userName = userName[itemIndex]
                    cell.content = content[itemIndex]
                    cell.rank = rank[itemIndex]
                    cell.bookName = bookName[itemIndex]
                    cell.stone = stone[itemIndex]
...
//        print(indexOnPageControl)
        let notificationName = Notification.Name(rawValue: "headImgClick")
        NotificationCenter.default.post(name: notificationName, object: indexOnPageControl)
    }
    func bookClick()  {
        let indexOnPageControl = pageControlIndexWithCurrentCellIndex(index: currentIndex())
//        print(indexOnPageControl)
        let notificationName = Notification.Name(rawValue: "bookLabClick")
        NotificationCenter.default.post(name: notificationName, object: indexOnPageControl)
    }
