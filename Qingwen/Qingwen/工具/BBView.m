...
-(float)fillBubbleViewWithButtons:(NSArray *)strings bgColor:(UIColor *)bgColor textColor:(UIColor *)textColor fontSize:(float)fsize{
    ...
    if (self.subviews.count == 0) {
        ...
        for (int xx = 0; xx < strings.count; xx++) {
            ...
            UIButton *oldButton = [UIButton new];
            if (xx > 0) {
                oldButton = self.bubbleButtonArray[xx - 1];
                if ((oldButton.frame.origin.x + (2*pad) + oldButton.frame.size.width + bSize.width + fsize) > UISCREEN_WIDTH) {
                ...
                if (xx == strings.count - 1) {
                    lasty = buttonRect.origin.y + bSize.height + fsize/2;
                    NSLog(@"bButton.frame = %f",buttonRect.origin.y + bSize.height + fsize/2);
                }
                
            }
            oldButton.frame = buttonRect;
            [self.bubbleButtonArray addObject:oldButton];
...
//            bButton.layer.shadowOffset = CGSizeMake(0.0f, 2.5f);
            bButton.layer.shadowRadius = 7.0f;
