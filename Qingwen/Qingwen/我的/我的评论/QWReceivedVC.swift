...
class QWReceivedVC: QWBaseVC,UITableViewDelegate,UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        if (QWSize.screenHeight() == 812.0) {
            tableView?.frame = CGRect(x:0,y:64+24,width:QWSize.screenWidth(),height:QWSize.screenHeight()-64-24)
            
        }
        else{
            tableView?.frame = CGRect(x:0,y:64,width:QWSize.screenWidth(),height:QWSize.screenHeight()-64)
...
//        let userId :String = QWGlobalValue.sharedInstance().nid!.stringValue
        let url = "\(QWOperationParam.currentBfDomain())/v3/post/" + uid! + "/recive_log/"
    ...
    override func getMoreData()  {
        ...
        var currentUrl = "\(QWOperationParam.currentBfDomain())" + nextStr!
