...
//  Qingwen
//
//  Created by mdk mdk on 2019/5/14.
...
#import "QWMyVIPVC.h"
@interface QWFreeZoneVC ()<UITableViewDelegate,UITableViewDataSource,SDCycleScrollViewDelegate>
{
    NSArray *titleArray;
...
@implementation QWFreeZoneVC

- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.title = @"免费专区";
...
-(void)createHeadView{
    ...
    titleLab.text = @"了解免费专区及激励计划";
    
    titleLab.frame = CGRectMake(10, 10.7, [QWSize autoWidth:@"了解免费专区及激励计划" width:500 height:12 num:12], 12);
    
    [topupBtn setTitle:@"激励计划" forState:0];
    topupBtn.frame = CGRectMake(kMaxX(titleLab.frame), 10.7, [QWSize autoWidth:@"激励计划" width:500 height:12 num:12]+10, 12);
...
-(void)topupBtnClick:(UIButton *)btn{
    [[QWRouter sharedInstance] routerWithUrl:[NSURL URLWithString:@"https://www.iqing.com/info/#/contribute"]];
}
...
-(void)getDataWithLocate:(NSString *)locate withorder_by:(NSString*)order_by{
    ...
    NSMutableDictionary *params = [NSMutableDictionary new];
    [self showLoading];
    NSString *url = [NSString stringWithFormat:@"%@/spbook/?locate=%@&order_by=%@&type=free&offset=0&limit=100",[QWOperationParam currentDomain],locate,order_by];
...
-(void)getbanner{
    ...
    NSMutableDictionary *params = [NSMutableDictionary new];
    [self showLoading];
    NSString *url = [NSString stringWithFormat:@"%@/recommend/?type=14&channel=1",[QWOperationParam currentDomain]];
    QWOperationParam *pm = [QWInterface getWithUrl:url params:params andCompleteBlock:^(id  _Nullable aResponseObject, NSError * _Nullable anError) {
        [self hideLoading];
        if (aResponseObject && !anError) {
