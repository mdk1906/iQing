...
class QWBookHistoryVC: QWBaseListVC {
    ...
    override func getData() {
        performInThreadBlock {[weak self] () -> Void in
            if let weakSelf = self {
                let books = BookCD.mr_findAllSorted(by: "lastReadTime", ascending: false, with: NSPredicate(format: "game == false && read == true"), in: QWFileManager.qwContext()) as! [BookCD]
                let page = PageVO()
                page.results = books
